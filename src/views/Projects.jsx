...
export default function Projects() {
  const { db, t, L, money, fmtDate, relDay, empById, search, openEditor, removeItem } = useStore()
            ...
            return (
              <motion.div key={p.id} className="pcard" variants={item} whileHover={{ y: -4 }} layout exit={{ opacity: 0, scale: 0.95 }}>
                ...
                <div className="row-actions" style={{ marginTop: "auto" }}>
                  ...
                  <button className="iconbtn del" onClick={() => confirm(t("confirmDel")) && removeItem("project", p.id)}><Icon name="trash" size={16} /></button>
                </div>
              </motion.div>
            )
          })}
